...
echo svg($points);
function svg($points, $width = 600, $height = 150, $stroke = "#1976D2", $linewidth = 4) {
	$min = min($points);
	$max = max($points);
	$max = $max * 1.0005;
	$min = $min * 0.9995;
	...
	$polyline = "<polyline fill='none' stroke-linejoin='round' stroke='{$stroke}' stroke-width='{$linewidth}' points='{$out}'/>";
